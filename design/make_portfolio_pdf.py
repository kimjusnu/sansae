...
def project(title, meta, bullets, stack):
    parts = [Paragraph(title, S["h1"]), Paragraph(meta, S["meta"])]
    parts += [bullet(b) for b in bullets]
    parts.append(Paragraph(f"<font color='#5a6478'>기술:</font> {stack}", S["meta"]))
    return KeepTogether(parts)

doc = SimpleDocTemplate(
    r"C:\Users\junsu\sansae\job-change\포트폴리오-김준수.pdf",
    pagesize=A4, leftMargin=17 * mm, rightMargin=17 * mm, topMargin=15 * mm, bottomMargin=14 * mm,
...
story.append(Paragraph(
    ...
    "기획부터 배포·운영까지 혼자 끝까지 가져가 본 경험이 반복적으로 있습니다.", S["intro"]))
story.append(hr(1))

# ── 1. Componique
story.append(project(
    "1. Componique — 오픈소스 UI 컴포넌트 라이브러리  [npm 주간 최대 127,000 다운로드]",
...
# ── 2. My Feed
story.append(project(
    "2. My Feed — 축산 IoT 모니터링·주문 SaaS (에임비랩, 재직 중)  [AI 분류 오류율 10%→3%]",
...
# ── 3. 홈페이지 리뉴얼
story.append(project(
    "3. 에임비랩 홈페이지 리뉴얼 — 투자자 사이트를 B2B 채널로  [고객 문의 월 1건 → 월 9건]",
...
# ── 4. StartupQT
story.append(project(
    "4. StartupQT — 창업 교육 퀴즈 저작·검수 SaaS (더이노베이터스 인턴)  [무중단 배포 파이프라인 단독 구축]",
...
# ── 5. 기타
story.append(project(
    "5. Eat Fit — AI 식단 관리 PWA  [SW 캡스톤디자인 우수상]",
...
story.append(project(
    "6. Wairi — 기업 홈페이지 Vue→Next.js 마이그레이션  [부트캠프 우수상 · 팀 리더]",
...
doc.build(story)
print("OK: portfolio PDF generated")
