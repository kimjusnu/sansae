...
`src/persona-ko.js`와 `src/persona-en.js`의 `PROFILE`을 **둘 다** 고치고 다시 배포합니다.
한쪽만 고치면 방문자가 고른 언어에 따라 서로 다른 이력이 나갑니다.

프롬프트는 언어당 하나입니다. 한국어 이력서를 주고 영어로 답하라고 시켰더니 고유명사를
즉석에서 번역했고, "방문자 문장은 번역해 주지 않는다"는 거절이 "영어를 쓰지 않는다"로
번져 버렸습니다. 두 프롬프트를 나누니 둘 다 사라졌습니다. 어느 쪽이든 "질문한 언어로
답한다"는 규칙은 그대로라, 영어 화면에서 한국어로 물어도 한국어로 답합니다.

클라이언트가 보내는 `lang` 필드가 어느 이력서를 쓸지 정합니다. 값이 없거나 이상하면
한국어로 떨어집니다.
